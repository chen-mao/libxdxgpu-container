...
/** 
 * Support for xdxctsmi 
*/

/**
 * 该结构体用于存放驱动版本相关信息
 */
typedef struct xdx_driver_ver_st 
{
    char DDK[10];                                   //!< DDK版本
    char KMD[10];                                   //!< KMD版本
    char UMD[10];                                   //!< UMD版本
...
typedef struct xdxml_pci_info_st
{
    ...
    uint64_t func;
    //unsigned int pci_device_id;        //!< The combined 16-bit device id and 16-bit vendor id

    // Added in NVML 2.285 API
    //nsigned int pciSubSystemId;     //!< The 32-bit Sub System Device ID
    char pci_dbdf[64];
    char bus_id[XDXML_DEVICE_PCI_BUS_ID_BUFFER_SIZE]; //!< The tuple domain:bus:device.function PCI identifier (&amp; NULL terminator)
} xdxml_pci_info_t;
...
typedef struct xdx_device_st *xdx_device_t;
 ...
 *
 * @return
 *         - \ref XDXML_SUCCESS       version  successful completion          
 */
...
/**
 * 获取DDK、KMD、UMD驱动版本信息，并存储至 \ref driver_ver 结构体中
 * 
 * Note: xdx_driver_ver_t是结构体指针类型，请在使用函数前提前为driver_ver变量分配内存空间
 * 如：xdx_driver_ver_t driver_ver_t = (xdx_driver_ver_t)malloc(sizeof(struct xdx_driver_ver_st));
 * 
 * @param device                        A structure used to save device information
 * @param mode                          A enum para used to indicate if display is connected or not         
 * 
 * @return
 *         - \ref XDXML_SUCCESS         successful completion          
 */
xdxml_return_t xdxml_device_get_driver_version(xdx_device_t device, xdx_driver_ver_t driver_ver);
 ...
 */
xdxml_return_t xdxml_device_get_pci_info(xdx_device_t device, xdxml_pci_info_t *pci);
