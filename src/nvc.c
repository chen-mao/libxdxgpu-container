...
nvc_init(struct nvc_context *ctx, const struct nvc_config *cfg, const char *opts)
{
        ...
        // If we are on not a platform with dxcore we will load the xdxct kernel modules and
        // use the xdxct libraries directly. If we do have access to dxcore, we will
        ...
        log_infof("flags:%d, OPT_LOAD_KMODS: %d", flags, OPT_LOAD_KMODS);
